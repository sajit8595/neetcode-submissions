class Solution:
    ...
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        dp = [[False for ind2 in range(self.s2Len + 1)] for ind1 in range(self.s1Len + 1)]
        dp[self.s1Len][self.s2Len] = True

        for ind1 in range(self.s1Len, -1, -1):
            for ind2 in range(self.s2Len, -1, -1):
                if ind1 == self.s1Len and ind2 == self.s2Len:
                    dp[self.s1Len][self.s2Len] = True
                    continue
                ...
                if ind1 < self.s1Len and self.s1[ind1] == self.s3[ind3]:
                    recurInd1 = dp[ind1+1][ind2]
                    if recurInd1:
                        dp[ind1][ind2] = True
                        continue
                
                if ind2 < self.s2Len and self.s2[ind2] == self.s3[ind3]:
                    recurInd2 = dp[ind1][ind2+1]
                    if recurInd2:
                        dp[ind1][ind2] = True
                        continue
